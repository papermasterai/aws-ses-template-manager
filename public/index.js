$(document).ready(() => {
  ...
  var getTemplates = function (nextToken, num = 0) {
    $.get(`/list-templates?region=${localStorage.getItem('region')}${nextToken ? `&NextToken=${encodeURIComponent(nextToken)}` : ''}`, function (data) {
      ...
      const result = templatesArr.filter((item) => {
        ...
        if (type === 'other') return !item.Name.startsWith('PM') && !item.Name.startsWith('quancheng');
  ...
  if (!localStorage.getItem('region')) {
    localStorage.setItem('region', 'ap-northeast-2');  //default region if none set
    localStorage.setItem('type', 'all');  //default region if none set
  } else {
    $('#regionSelector').val(localStorage.getItem('region')); //always ensure the select region dropdown matches localstorage region
    $('#typeFilter').val(localStorage.getItem('type'));
  }
  ...
  $('#typeFilter').change(function () {
    const type = $(this).val(); //get changed to selection
